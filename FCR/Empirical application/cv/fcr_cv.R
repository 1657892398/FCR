...
custom_cv_fcr <- function (x, y, C, m, lamda, gamma=c(1,1)) {
  ...
  for (repeats in 1:n_repeats) {
    ...
    for (fold in 1:n_folds) {
      ...
      # 在此基础上Convert soft clustering matrix to hard clustering
      ...
      Mse <- compute_mse(x_valid,y_valid,w_pre(x_valid,best_clr$mu) , lasso_beta)
...
all_mse <- custom_cv_fcr(x, y, C, m, lamda, gamma=c(1,1))
all_mse
mean(all_mse)

# 保存当前所有R对象
save.image("CV0.RData")
