...
# 主函数
setwd("C:/Users/dell/Desktop/写论文/r语言代码版本")
...
# Define the function
FCR <- function(x_data, y_data, C, m, lamda, gamma=c(1,1), nstart = 1, eps = 0.0001) {
  # Prepare data
  N <- nrow(x_data)
  p <- ncol(x_data)
  lamda <- lamda / gamma[1]
...
custom_cv_fcr <- function (x, y, C, m, lamda, gamma=c(1,1)) {
  ...
  for (repeats in 1:n_repeats) {
    ...
    for (fold in 1:n_folds) {
      ...
      rmse_pre <- compute_rmse(x_valid,y_valid,w_pre(x_valid,best_clr$mu) ,best_clr$coefficients)
